...
__codes("allow") = {
	...
	"reason": sprintf(
		...
		["zohaib.khan@swisscom.com", allowing_roles[0], "default", allowing_roles[0], input.request.object.operation, input.request.kind.kind],
...
__codes("no_such_action") = {"reason": sprintf(
	...
	[input.request.object.operation, input.request.kind.kind, input.request.kind.kind, debug_utils.format_array(data.resource_types[input.resource.type].actions)],
...
__codes("no_role_in_tenant") = {"reason": sprintf(
	...
	[
		input.request.object.operation, "default",
